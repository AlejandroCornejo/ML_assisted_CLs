...
def LoadHpromGprModel(
    ...
    ecm = np.load(os.path.join(hprom_gpr_dir, "ecm_weights_all.npz"))
...
def RunHpromGprBatchSimulation(
    parameters,
    phi_p,
    phi_s,
    free_dofs,
    ...
    if phi_p_ref.shape[0] != free_dofs_ref.size:
        raise RuntimeError(
            f"[HPROM-GPR] Basis/free_dofs mismatch: phi rows={phi_p_ref.shape[0]} "
    ...
    vec_full_assembler = VectorizedAssembler(mp, n_total_dof, eq_id_map, log_label="HPROMGPRFullSyncAssembler")
    ...
    print(f"  [HPROM-GPR] Homogenization reference measure A0 (full mesh): {hom_reference_measure:.6e}")
    ...
    if not using_weighted_hom:
        raise RuntimeError(
            "[HPROM-GPR] ECM homogenization weights are required (w_eps/w_sig not available)."
        )
    print("  [HPROM-GPR] Using ECM-weighted homogenization (w_eps / w_sig).")
    ...
    if using_hrom_mesh:
        print("  [HPROM-GPR] Using reduced HROM mesh residual weights (w_res_hrom).")
    t_map = 0.0
    t_assembly = 0.0
    t_projection = 0.0
    t_solve = 0.0
    ...
    for step in range(1, n_steps_total + 1):
        ...
        while it < max_its:
            ...
            if not _is_finite(J_gpr):
                print("  [HPROM-GPR] WARNING: non-finite GPR Jacobian detected.")
                nonfinite_detected = True
                break

            u_fluc = w0_const + phi_p_eff @ q_p + phi_s @ q_s
            if not _is_finite(u_fluc):
                print("  [HPROM-GPR] WARNING: non-finite reconstructed displacement detected.")
                nonfinite_detected = True
                break
            ...
            if not _is_finite(J_manifold):
                print("  [HPROM-GPR] WARNING: non-finite manifold Jacobian detected.")
                nonfinite_detected = True
                break
            ...
            if (not _is_finite(rhs_hp)) or (not _is_finite(K_hp.data)):
                print("  [HPROM-GPR] WARNING: non-finite full residual/stiffness detected.")
                nonfinite_detected = True
                break
            ...
            if (not _is_finite(r_r)) or (not _is_finite(K_r)):
                print("  [HPROM-GPR] WARNING: non-finite reduced residual/stiffness detected.")
                nonfinite_detected = True
                break

            res_norm = float(np.linalg.norm(r_r))
            if not np.isfinite(res_norm):
                print("  [HPROM-GPR] WARNING: non-finite reduced residual norm detected.")
                nonfinite_detected = True
                break
            ...
            if not _is_finite(dq_p):
                print("  [HPROM-GPR] WARNING: non-finite reduced update detected.")
                nonfinite_detected = True
                break
        ...
        if not _is_finite(u_fluc_final):
            ...
            u_fluc_final = w0_const + phi_p_eff @ q_p + phi_s @ q_s_final
        if not _is_finite(u_fluc_final):
            raise RuntimeError("HPROM-GPR accepted state is non-finite after rollback.")
    ...
    print(
        f"\n[HPROM-GPR] Timing: map={t_map:.3f}s, assembly={t_assembly:.3f}s, "
    ...
    print(
        f"  [HPROM-GPR] Hyper-assembly stats: calls={int(hr_stats['calls_total'])}, "
    ...
    np.savez(os.path.join(out_dir, "hprom_gpr_timing_stats.npz"), **{
        k: np.array([v], dtype=float) for k, v in timing_stats.items()
    })
    with open(os.path.join(out_dir, "hprom_gpr_timing_stats.txt"), "w", encoding="utf-8") as f:
        for k, v in timing_stats.items():
            f.write(f"{k}={v}\n")

    tag = f"trajectory_{trajectory_index}" if trajectory_index is not None else "hprom_gpr_run"
