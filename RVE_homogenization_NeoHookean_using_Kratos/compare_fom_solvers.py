...
"""
Compare KRATOS, FOM, and FOM-VECTORIZED on the same Stage 8/10/11 trajectory.

Default behavior:
- builds the same safe test path used in stage workflows
- runs/loads 3 solvers on exactly that path
...
import fom_vectorized_solver_rve as fom_vec_solver
from kratos_solver_rve import RunKratosBatchSimulation


METHOD_ORDER = ["KRATOS", "FOM", "FOM-VECTORIZED"]
...
def _plot_comparison(all_eps, all_sig, out_dir, timings, reference_method="FOM"):
    ...
    for i, label_sig, label_eps, suffix in comp_labels:
        ...
        plt.title(f"3-Solver Benchmark: {label_sig}")
    ...
    plt.title(r"3-Solver Benchmark: $\sigma_{eq}$ vs $\varepsilon_{eq}$")
...
def _run_fom_vectorized(strain_path, reference_amplitude, out_dir):
    parameters = fom_vec_solver.setup_kratos_parameters("rve_geometry")
    t0 = time.perf_counter()
    eps, sig = fom_vec_solver.RunFomBatchSimulation(
        parameters,
        out_dir=out_dir,
        save_plot=False,
        strain_path=strain_path,
...
def run_compare(
    ...
    print("=" * 70)
    print("  KRATOS vs FOM vs FOM-VECTORIZED")
    print("=" * 70)
    ...
    cache = {
        ...
        "FOM-VECTORIZED": (
            os.path.join(out_dir, "fom_vectorized_strain.npy"),
            os.path.join(out_dir, "fom_vectorized_stress.npy"),
        ),
    }
    ...
    forces = {
        ...
        "FOM": bool(run_fom),
        "FOM-VECTORIZED": bool(run_fom_vect),
    }

    subdirs = {
        "KRATOS": os.path.join(out_dir, "kratos_run"),
        "FOM": os.path.join(out_dir, "fom_run"),
        "FOM-VECTORIZED": os.path.join(out_dir, "fom_vectorized_run"),
    }
    ...
    run_or_load("FOM-VECTORIZED", _run_fom_vectorized)
    ...
    if "FOM" in timings and "FOM-VECTORIZED" in timings:
        timings["speedup_vec_vs_fom"] = timings["FOM"] / max(timings["FOM-VECTORIZED"], 1e-30)
    ...
    print("  Summary")
    print("=" * 70)
    for method in ["KRATOS", "FOM-VECTORIZED"]:
        err, n = errors[method]
    ...
    if "speedup_vec_vs_fom" in timings:
        print(f"  Speedup (FOM / FOM-VECTORIZED): {timings['speedup_vec_vs_fom']:.3f}x")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("KRATOS vs FOM vs FOM-VECTORIZED\n")
        f.write("=" * 44 + "\n")
        ...
        if "speedup_vec_vs_fom" in timings:
            f.write(f"speedup_vec_vs_fom: {timings['speedup_vec_vs_fom']:.12e}\n")
        if "speedup_kratos_vs_fom" in timings:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare KRATOS, FOM, and FOM-VECTORIZED on Stage 8/10/11 trajectory"
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default="compare_3solvers_stage_path_results",
    ...
    parser.add_argument("--run-fom-vect", action="store_true", help="Force recomputation of FOM-VECTORIZED.")
    parser.add_argument(
    ...
    if args.run_all:
        ...
        args.run_fom = True
        args.run_fom_vect = True

    run_compare(
        out_dir=args.out_dir,
        run_kratos=args.run_kratos,
        run_fom=args.run_fom,
        run_fom_vect=args.run_fom_vect,
