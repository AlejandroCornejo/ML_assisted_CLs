...
def run_mawecm_pruning(A_blocks, b_blocks, z_ini, w_ini, q_train, options=None):
    ...
    # Graph laplacian is supplied by caller in options.
    if "K_graph" not in opts:
        raise ValueError("options must include K_graph (graph Laplacian).")
    ...
    while int(i_cand.size) > int(n_stop):
        ...
        graph_attempts += 1
        ok_g, W_new, i_cand_new, removed_idx, graph_diag = _try_graph_pass(
            W=W,
            A_blocks=A_blocks,
            b_blocks=b_blocks,
            i_cand=i_cand,
        ...
        removed.append(int(removed_idx))
        removed_history.append(int(removed_idx))
        stage_history.append("graph_active_set")
        active_counts.append(int(i_cand.size))
