...
def _resolve_selection_indices(data, selection_key):
    key = str(selection_key).strip()
    if key in data:
    ...
    parts = []
    for k in ("Z_eps", "Z_sig"):
        if k in data:
            parts.append(np.asarray(data[k], dtype=np.int64).reshape(-1))
    for base in ("eps", "sig"):
        for comp in range(3):
            k = f"Z_{base}_{comp}"
            if k in data:
                parts.append(np.asarray(data[k], dtype=np.int64).reshape(-1))
    if not parts:
        raise KeyError("Z_hom_union requested, but no Z_eps/Z_sig component supports were found.")
...
def main():
    ...
    p.add_argument(
        "--selection-key",
        default="Z_union",
        choices=["Z_res", "Z_union", "Z_eps", "Z_sig", "Z_hom_union"],
    ...
    try:
        selected_idx = _resolve_selection_indices(data, args.selection_key)
    except KeyError as exc:
        raise KeyError(
            f"{args.selection_key} not found in {ecm_file}. Available keys: {sorted(data.keys())}"
        ) from exc
    ...
    data_out = dict(data)
    if str(args.selection_key) == "Z_hom_union":
        data_out["Z_hom_union"] = np.asarray(selected_idx, dtype=np.int64)
    ...
    if args.save_selection_image:
        ...
        if bool(int(args.save_extra_selection_images)):
            for key in _split_keys_csv(args.extra_selection_keys):
                try:
                    key_idx = _resolve_selection_indices(data_out, key)
                except KeyError:
                    continue
                if key_idx.size == 0:
                    continue
