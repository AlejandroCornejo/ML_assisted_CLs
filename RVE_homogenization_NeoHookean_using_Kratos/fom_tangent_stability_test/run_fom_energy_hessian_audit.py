...
PANN_DATA = RVE_DIR / "PANN_D4" / "data" / "alltraj_stage10_direct_energy.npz"
STAGE1_DIR = RVE_DIR / "stage_1_training_set_fom"
STAGE10_DIR = RVE_DIR / "stage_10_hprom_ann_ls_results_mawecm_res_eps_sig_phase1to40_phase2to10_sum990_ann_hrom"
...
def make_parameters() -> KM.Parameters:
    ...
    with open(RVE_DIR / "ProjectParameters.json", encoding="utf-8") as parameter_file:
    ...
    mesh_base = str(RVE_DIR / "rve_geometry")
    fom.SetInputMeshFilename(parameters, mesh_base)
    material_parts = fom.DetectMaterialSubModelParts(str(RVE_DIR / "rve_geometry.mdpa"))
    parameters = fom.ConfigureElementModelerForMaterialParts(parameters, material_parts)
    fom.SetMaterialsFilename(parameters, str(RVE_DIR / "StructuralMaterials.json"))
    return parameters
...
def source_paths(source: str) -> tuple[Path, Path]:
    ...
    if source == "stage10":
        return STAGE10_DIR / "single_run_applied_strain.npy", STAGE10_DIR / "single_run_U.npy"
