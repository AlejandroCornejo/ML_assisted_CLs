...
def _evaluate_rbf_origin_terms(rbf_model, n_primary):
    return _evaluate_rbf_qs_and_jac(np.zeros(n_primary, dtype=float), rbf_model, n_primary)
...
def _fit_qp_gauss_newton(
    ...
    q_s_raw, j_qs_qp_raw = _evaluate_rbf_qs_and_jac(q, rbf_model, n_primary)
...
def main():
    ...
    print("[Info] Manifold correction active in Stage 9a-RBF: N(0)=0 and J(0)=0.")
    ...
    fit_rel_after = []
    fit_iters = []
    q0_const, j0_const = _evaluate_rbf_origin_terms(rbf_model, n_primary)
        ...
        for k in idx_union:
            ...
            if ks in set_res:
                ...
                if fit_mode == "gauss_newton":
                    ...
                    q_p = q_p_init
                    q_s_raw, j_qs_qp_raw = _evaluate_rbf_qs_and_jac(q_p, rbf_model, n_primary)
