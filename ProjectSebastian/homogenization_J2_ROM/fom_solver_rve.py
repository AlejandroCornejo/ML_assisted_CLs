...
from j2_plane_stress_plastic_strain_rve_simo_optimized import VonMisesIsotropicPlasticityPlaneStress
...
def main():
    ...
    all_U_histories = []  # POD: displacement snapshots
    batch = 0
    ...
    while theta <= 50.0 + 1e-8:
        while phi <= 50.0 + 1e-8:
            ...
            # NEW: global mesh arrays
            conn, B_all, w_all, area_all, pattern_I, pattern_J = precompute_mesh_arrays(mp, idx_ux, idx_uy)
            ...
            B_glob = assemble_global_B_vec(conn, B_all, n_dof)
            ...
            while time < end_time - 1e-12:
                ...
                for it in range(max_newton_it):
                    ...
                    for ls_it in range(max_ls_it):
                        ...
                        _, f_int_trial, _, _ = assemble_K_and_fint_vec(
                            mat, conn, B_all, w_all,
                            eps_p_n, alpha_n,          # still the committed state
                ...
                # Commit plastic state (this replaces mat.Commit())
                eps_p_n[:] = eps_p
                alpha_n[:] = alpha
            ...
            # POD: finalize displacement history for this batch
            U_history = np.stack(U_history, axis=0)  # (n_steps+1, n_dof)
            all_U_histories.append(U_history)
    ...
    # POD: displacement snapshots only (no extra metadata)
    U_tensor = np.stack(all_U_histories, axis=0)  # (n_batches, n_steps+1, n_dof)
    np.savez("data_set/all_displacement_snapshots.npz", U=U_tensor)

    print("Results stored in:")
    print("  data_set/all_stress_histories.npz")
    print("  data_set/all_strain_histories.npz")
    print("  data_set/all_displacement_snapshots.npz")
