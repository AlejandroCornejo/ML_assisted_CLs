...
class RVE_homogenization_dataset_generator(analysis_stage.AnalysisStage):
    ...
    def ApplyBoundaryConditions(self):
        ...
        I kept exactly Alejandro's logic, but Kratos won't solve anything,
...
while theta <= 360.0 + 1e-8:
    while phi <= 360.0 + 1e-8:
        ...
        while time < end_time - 1e-12:
            ...
            # 2) Our linear solver
            # ------------------------------------------------------------
            u, res_free = solve_linear_system_with_dirichlet(solver_state, mp, idx_ux, idx_uy, step_index=0)
            ...
            # ------------------------------------------------------------
            # 4) Print summary for this step
            # ------------------------------------------------------------
            print(f"[Batch {batch} | Step {step:03d}] " f"||res_free||={res_free:.3e}")
