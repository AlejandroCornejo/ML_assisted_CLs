...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--component-stress-weight", type=float, default=None, help=f"Overrides the default {COMPONENT_STRESS_WEIGHT}."
    )
...
def main() -> None:
    ...
    component_stress_weight = (
        COMPONENT_STRESS_WEIGHT if args.component_stress_weight is None else args.component_stress_weight
    )
    ...
    # Derived from --output-name (not just --kind) so that a second run of the
    # same --kind with a distinct --output-name cannot silently overwrite the
    # history/resume-state/summary of an earlier run of that kind.
