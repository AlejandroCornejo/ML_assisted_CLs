#!/usr/bin/env python3
"""Evaluate the final (component-stress-weight=0) polyconvex PANN on Stage--10.

Companion to evaluate_anisotropic_pann.py, restricted to the single certified
polyconvex model.  Reports energy, second-Piola stress (components and the
plane-stress von Mises equivalent), and the same architectural audits
(reference state, objectivity, D4-not-imposed, polyconvexity falsification
attempts, volumetric barrier).  Also loads the officially recorded direct
HPROM-ANN reconstruction metrics on the same Stage-10 path for comparison.
"""
...
HPROM_REFERENCE = HERE.parent / "data" / "stage10_hprom_direct_reference.json"
RESULT_DIR = HERE / "results"


def sigma_eq(stress: np.ndarray) -> np.ndarray:
    """Plane-stress von Mises equivalent of [S11, S22, S12]."""
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", default="PANN_anisotropic_polyconvex_final_claude.pt")
    parser.add_argument("--output-prefix", default="polyconvex_final_claude")
    ...
    model, strain_scale, energy_scale, checkpoint = load_anisotropic_polyconvex(
    ...
    result = {
        ...
        "icnn_widths": checkpoint["model_configuration"]["icnn_widths"],
