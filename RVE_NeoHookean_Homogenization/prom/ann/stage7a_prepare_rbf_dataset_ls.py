# NOTE: despite the "rbf" in this filename (kept unchanged so
# stage7a_prepare_ann_dataset_ls.py's `runpy.run_module("stage7a_prepare_rbf_dataset_ls", ...)`
# call keeps working), this module builds the shared Delaunay-based LS
# secondary-coordinate closure q_s(q_m) from POD/FOM snapshots. It has no
# RBF-specific logic and is the dataset-prep step this ANN family actually
# depends on -- see README.md.
import os
import sys
import argparse
...
while not (_HERE / "core").is_dir():
    _HERE = _HERE.parent
if str(_HERE / "core") not in sys.path:
...
def prepare_rbf_dataset_least_squares(
    ...
    train_dir=str(_HERE / "trajectories" / "stage_1_training_set_fom"),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--basis-dir", type=str, default=str(_HERE / "pod" / "stage_2_pod_rve"), help="POD basis directory.")
    parser.add_argument("--train-dir", type=str, default=str(_HERE / "trajectories" / "stage_1_training_set_fom"), help="Training snapshots directory.")
