#!/usr/bin/env python3
"""Stage 2: full (Newton-corrected) HPROM-ANN, extracted from
RunHpromAnnBatchSimulation's inner `while it < max_its` loop
(hprom/ann/hprom_ann_solver_rve.py lines ~1698-2020) into a standalone,
build-once/evaluate-many class.

Faithfully transcribes the original's dynamic-MAW-ECM residual path
(dynamic_residual_weights=True, the branch this project's own trained
ecm_weights_all.npz actually uses) -- every convergence heuristic is
kept exactly as written, since any deviation would converge to a
slightly different q_p and break exact reproduction of the paper's own
already-reported numbers. The only deliberate simplification: always
uses the torch-autodiff decoder Jacobian (skips the "manual NumPy MLP"
fast-path optimization), which the original code itself validates
against the torch path and only uses when they agree to tight
tolerance -- so this is a performance simplification, not a
correctness one.

Unlike Stage 1's direct mode (memoryless, mu-affine init only), this
mode's reference run used qp_init_mode="continuation": q_p's initial
guess for step>1 is the PREVIOUS step's converged q_p. evaluate() takes
an explicit q_prev, matching Cook's-membrane's own load-ramp structure
(each Gauss point has its own previous-load-step converged strain to
warm-start from).

See /home/kratos/.claude/plans/jazzy-meandering-pebble.md (Stage 2).
"""
...
class HpromAnnIterativeLaw:
    """Build-once / evaluate-many extraction of full, Newton-corrected
    HPROM-ANN (dynamic MAW-ECM residual weights, the mode this
    project's own trained ecm_weights_all.npz uses)."""
    ...
    def __init__(
        ...
        self.vec_assembler = VectorizedAssembler(mp, n_total_dof, eq_id_map, log_label="HpromAnnIterativeLaw")
        ...
        free_dofs, dir_dofs, basis_rows = ResolveActiveFreeDofsAndBasisRows(
            ...
            full_mesh_base=full_mesh_base, solver_label="HpromAnnIterativeLaw",
        )
        ...
        # --- residual-weight (dynamic MAW-ECM) setup, the piece direct mode skips ---
        ...
        if miss_res.size:
            print(f"[HpromAnnIterativeLaw] WARNING: residual MAW support lost {miss_res.size} full-mesh indices.")
        ...
        # --- homogenization-weight setup, same as Stage 1 ---
        ...
        # --- affine lifting + decoder constants, same as Stage 1 ---
    ...
    # ---- decoder helpers (torch-autodiff path only; see module docstring) ----

    def _eval_qs_only(self, qp_vec):
        qp_t = torch.from_numpy(np.asarray(qp_vec, dtype=float).astype(np.float32)).unsqueeze(0).to(self.device)
        with torch.no_grad():
            q_s_map = self.ann_model(qp_t)
        return q_s_map.detach().cpu().numpy().reshape(-1)

    def _eval_qs_and_jac(self, qp_vec, e_vec):
        qp_t = torch.from_numpy(np.asarray(qp_vec, dtype=float).astype(np.float32)).reshape(-1).to(self.device)
    ...
    def _compute_weighted_decoder_hessian(self, qp_vec, output_weights):
        qp_t = torch.from_numpy(np.asarray(qp_vec, dtype=float).astype(np.float32)).to(self.device)
        weights = torch.from_numpy(np.asarray(output_weights, dtype=float).astype(np.float32)).to(self.device)
        with torch.enable_grad():
            qp_in = qp_t.clone().detach().requires_grad_(True)
    ...
    def evaluate(self, E, q_prev=None, step_index=1):
        """Public entry point: guards the RVE-internal assembler calls inside
        _evaluate_impl against an outer (Cook-level) monkeypatch of the same
        module-level material-law name -- see _material_law_guard_claude.py."""
    ...
    def _evaluate_impl(self, E, q_prev=None, step_index=1):
        """Runs the full inner Newton loop at one macro strain E, warm-started
        from q_prev (the previous load step's converged q_p, or None/zeros for
        a first query), and returns (hom_eps, hom_sig, q_p_converged, n_iters,
        converged: bool). Faithfully transcribes RunHpromAnnBatchSimulation's
        dynamic-MAW-ECM inner loop (hprom_ann_solver_rve.py ~1698-2020)."""
    ...
    def _hom_weights_and_jacobian(self, maw_models, E):
        ...
        evaluate_with_tangent; _evaluate_impl above never calls this."""
        if self.maw_hom_componentwise:
            results = [
    ...
    def _residual_jacobian_at(self, q_p, E, disp_base, u_aff_free):
        """One forward (no-update) pass of _evaluate_impl's inner-loop
        residual/Jacobian block (lines ~371-430 above), evaluated AT a given
        q_p rather than iterated. Duplicated rather than factored out of
        _evaluate_impl on purpose -- that method is already verified against
        the saved reference and its faithful transcription of every original
        convergence heuristic is deliberately left untouched; this is a pure
        addition. Returns (J_manifold, K_r, w_res_iter, u_eq_curr, r_full)."""
        q_s_map, J_ann_raw = self._eval_qs_and_jac(q_p, E)
        q_s = q_s_map - self.q0_const_np - self.J0_const_np @ q_p
        J_ann = J_ann_raw - self.J0_const_np
        u_fluc = self.w0_const_np + self.phi_p_eff_np @ q_p + self.phi_s @ q_s
    ...
    def dqp_dE_at(self, q_p, E, disp_base, u_aff_free, J_manifold=None, K_r=None, w_res_iter=None):
        """Implicit-function-theorem sensitivity of the inner loop's own
        fixed point q_p*(E) (r_r(q_p*(E), E) = 0) to the macro strain E, at
        a q_p already known to satisfy (or closely approximate) that fixed
        point. Returns (dq_p/dE (n_primary,3), J_manifold, K_r, w_res_iter)
        so callers that already need J_manifold/K_r for the homogenization
        tangent do not recompute them twice.

        Derivation (see the module docstring in verify_hprom_ann_iterative_
        tangent_ift_claude.py for the full sign bookkeeping): with
        G(q_p,E):=r_r(q_p,E), dG/dq_p = -K_r exactly (K_r is already the
        curvature- and weight-Jacobian-corrected consistent tangent the
        inner loop assembles for its own Newton steps: dq_p_newton =
        K_r^{-1} @ r_r, i.e. K_r = -dr_r/dq_p by the standard Newton-update
        sign convention). E enters G only through the affine-lifting term
        (residual weights depend on q_p alone -- coord_label='q' -- and the
        decoder never sees E), so
            dG/dE|_{q_p} = J_manifold^T @ d(r_full)/dE|_{q_p}
        with d(r_full)/dE|_{q_p} built from the SAME weighted-scatter
        pattern the inner loop already uses for drw_dqp, substituting
        -(K_total_support @ du_local/dE) for -f_int_support (both use the
        support sub-assembler's own already-populated K_total/f_int
        buffers). The IFT then gives
            dq_p/dE = -(dG/dq_p)^{-1} @ (dG/dE) = K_r^{-1} @ dG/dE.
        """
        ...
        du_local_dE_support = du_aff_dE[local_eq_ids_support, :]  # (n_sel, n_local_dof, 3)
        K_total_support = support_assembler._K_total  # (n_sel, n_local_dof, n_local_dof)
        ...
        dG_dE = J_manifold.T @ dr_full_dE  # (n_primary, 3)
    ...
    def evaluate_with_tangent(self, E, q_prev=None, step_index=1):
        """Same (hom_eps, hom_sig, q_p, n_iters, converged) as evaluate(),
        plus the consistent macro tangents dEps_hom/dE, dSig_hom/dE (each
        3,3), via the implicit-function-theorem dq_p/dE from dqp_dE_at
        composed with the same outer/homogenization chain rule Stage 1's
        DHpromAnnDirectLaw.evaluate_with_tangent already uses (reused here
        via DHpromAnnDirectLaw._dhom_dE, a pure staticmethod with no
        Stage-1-specific state).

        Calls evaluate() (hence _evaluate_impl) FIRST to get the q_p that
        would actually be returned/used downstream, then re-evaluates the
        residual/Jacobian block at that q_p (via dqp_dE_at ->
        _residual_jacobian_at) rather than trusting whatever K_r/J_manifold
        the inner loop last held internally: on the quasi-converged/
        plateau-fallback path, _evaluate_impl returns q_p=best_q, which can
        differ from the LAST q_p the loop actually evaluated, so anything
        computed inside the loop's final iteration could be stale relative
        to the q_p that is actually returned and used to build the
        displacement field. Re-evaluating removes that risk at the cost of
        one cheap extra forward pass (not a full extra Newton solve)."""
        with true_neo_hookean_active():
            E = np.asarray(E, dtype=float).reshape(-1)
...
def get_law(**kwargs):
    """Lazily builds the one, process-wide HpromAnnIterativeLaw instance,
    defaulting to qp_init_mode='mu_affine' -- verified (see
    verify_hprom_ann_iterative_extraction_claude.py) to converge to the
    same fixed point as 'continuation' on the held-out trajectory, and
    memoryless: no q_prev needs to be tracked per Gauss point across
    Cook's-membrane Newton iterations or load steps."""
    global _DEFAULT_LAW
    if _DEFAULT_LAW is None:
        kwargs.setdefault("qp_init_mode", "mu_affine")
        _DEFAULT_LAW = HpromAnnIterativeLaw(**kwargs)
    return _DEFAULT_LAW


def hprom_ann_iterative_pk2_2d_vectorized(E_flat, young=None, poisson=None, h=1.0e-4):
    """Matches the (E_flat (N,3), young, poisson) -> (S (N,3), CC (N,3,3))
    signature Cook's VectorizedAssembler expects. Stress via
    HpromAnnIterativeLaw.evaluate (mu_affine init, q_prev=None each call
    -- no per-Gauss-point state to track); tangent via central finite
    differences, same convention as Stage 1."""
    law = get_law()
...
def hprom_ann_iterative_pk2_2d_vectorized_consistent(E_flat, young=None, poisson=None):
    """Same (E_flat (N,3), young, poisson) -> (S (N,3), CC (N,3,3)) contract
    as hprom_ann_iterative_pk2_2d_vectorized (young/poisson accepted but
    unused, signature compatibility only), but CC is the consistent
    (implicit-function-theorem) analytic tangent from
    HpromAnnIterativeLaw.evaluate_with_tangent instead of a central finite
    difference. Same mu_affine/memoryless convention (q_prev=None each
    call) as the finite-difference version above."""
    law = get_law()
    E_flat = np.asarray(E_flat, dtype=float)
    n = E_flat.shape[0]
    S = np.zeros((n, 3), dtype=float)
...
if __name__ == "__main__":
    print("[hprom_ann_iterative_law] building law instance ...")
    law = HpromAnnIterativeLaw(verbose=True)
    eps0, sig0, q0, n_it, conv = law.evaluate(np.array([0.05, 0.0, 0.0]))
    print(f"[hprom_ann_iterative_law] evaluate([0.05,0,0]) -> eps={eps0}, sig={sig0}, "
