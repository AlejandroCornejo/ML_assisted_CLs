#!/usr/bin/env python3
"""Force-controlled Cruciform check for the 3 cheap ROM/HPROM tiers
(Linear-HPROM, HPROM-ANN, D-HPROM-ANN), at the reference force level
(1.2551e9 N, matching ICNN's own reaction force at the standard
delta=1.2 displacement protocol, already verified self-consistent).
These tiers' continuation/warm-start machinery has never been exercised
under force control before -- this checks whether they behave sanely
at all (not yet a search for trouble, unlike the PANN sweep), before
deciding whether a full force-controlled table (including the much more
expensive FOM-FE2 row) is worth building."""
...
def main():
    ...
    for which in (which_linear, which_hprom_ann, which_dhprom):
        print(f"\n=== {which}, force-controlled, F={REF_FORCE:.4e} (reference level) ===", flush=True)
        t0 = time.time()
        try:
            res = run_newton_fe2_cruciform_force(
                which, n_body=6, n_arm_len=4, n_steps=N_STEPS, total_force_final=REF_FORCE,
            ...
            print(f"[{which}] fully_converged={res['fully_converged']}, ever_diverged={res['ever_diverged']}, "
                  ...
                  f"final tip_ux={res['step_log'][-1]['tip_ux_px_mean']:.4f}, "
                  f"tip_uy={res['step_log'][-1]['tip_uy_py_mean']:.4f}, "
    ...
    print("ROM_HPROM_FORCE_CHECK_DONE_MARKER", flush=True)
