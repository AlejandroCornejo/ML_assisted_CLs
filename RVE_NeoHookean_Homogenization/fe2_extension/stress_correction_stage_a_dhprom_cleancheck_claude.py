...
cook_results_dhprom_f64_consistent_claude.npz), reading back the converged full
...
OUT_NPZ = HERE / "stress_correction_stage_a_dhprom_cleancheck_result_claude.npz"
SCRATCH_DIR = HERE / "fom_query_scratch_dhprom_cleancheck"
...
def main() -> None:
    ...
    n = e_gp.shape[0]
    print(f"[dhprom-cleancheck] loaded e_gp {e_gp.shape}, s_gp {s_gp_saved.shape} from {COOK_NPZ.name}")

    # Probe n_dof from a throwaway zero-strain solve before allocating.
    _eps0, _sig0, u0 = solve_at_strain_full_field(np.zeros(3))
    n_dof = u0.shape[0]
    print(f"[dhprom-cleancheck] n_dof={n_dof}")
    ...
    dt = time.perf_counter() - t0
    print(f"[dhprom-cleancheck] DONE: {n} fresh cold-start solves, {dt:.1f}s total ({dt / n:.3f}s/state)")

    err = np.linalg.norm(sig_check - s_gp_saved) / max(np.linalg.norm(s_gp_saved), 1.0e-30)
    print(f"[dhprom-cleancheck] cross-check: fresh cold-start native (unweighted volume-average) "
          f"stress vs saved s_gp, relative L2 = {err:.6e}")
    if err > 1.0e-3:
        print("[dhprom-cleancheck] WARNING: cross-check error is larger than expected for a "
    ...
    print(f"[dhprom-cleancheck] saved to {OUT_NPZ}")
