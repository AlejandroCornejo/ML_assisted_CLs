#!/usr/bin/env python3
"""Stage B of the Cook's-membrane reaction-force unification, HPROM-ANN.
...
STAGE_A_NPZ = HERE / "stress_correction_stage_a_hprom_result_claude.npz"
OUT_NPZ = HERE / "stress_correction_stage_b_hprom_result_claude.npz"
...
def main() -> None:
    ...
    n = e_gp.shape[0]
    print(f"[stage-b-hprom] loaded q_p_all {q_p_all.shape}, e_gp {e_gp.shape} from stage A")
    ...
    print(f"[stage-b-hprom] dof partition check OK: n_free={free_dofs_pod.size}, n_dir={dir_dofs_pod.size}")
    ...
    print(f"[stage-b-hprom] ls decoder loaded: phi_p={phi_p.shape}, phi_s={phi_s.shape}, a_m={a_m.shape}")
    ...
    print(f"[stage-b-hprom] full-field reconstruction: {time.perf_counter() - t0:.2f}s for {n} states")

    t0 = time.perf_counter()
    stress_rf = gen.direct_stress_history(U_full, e_gp)
    dt = time.perf_counter() - t0
    print(f"[stage-b-hprom] reaction-force correction: {dt:.2f}s total ({dt / n:.4f}s/state)")
    ...
    print(f"[stage-b-hprom] diagnostic state 0: reconstructed full-mesh strain={eps_h0}, imposed e_gp={e_gp[0]}")

    err_vs_native = relative_l2(stress_rf, s_gp_native)
    print(f"[stage-b-hprom] reaction-force-corrected stress vs native (Z_sigma) stress: "
    ...
    print(f"[stage-b-hprom] saved to {OUT_NPZ}")
