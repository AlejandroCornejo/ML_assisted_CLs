...
while not (_HERE / "core").is_dir():
    _HERE = _HERE.parent
for _p in (_HERE / "core", _HERE / "prom" / "pod"):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))
_DEFAULT_FOM_DIR = str(_HERE / "trajectories" / "stage_1_training_set_fom")
...
def run_verification(idx, mesh="rve_geometry", fom_dir=_DEFAULT_FOM_DIR):
