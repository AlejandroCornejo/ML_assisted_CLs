...
while not (_HERE / "core").is_dir():
    _HERE = _HERE.parent
for _p in (_HERE / "core", _HERE / "prom" / "ann"):
...
SNAPSHOTS_DIR = str(_HERE / "trajectories" / "stage_1_training_set_fom")
BASIS_DIR = str(_HERE / "pod" / "stage_2_pod_rve")
ANN_DIR = "stage_7_ann_data"
OUT_DIR = "stage_9_ecm_dataset_ann"
SNAPSHOT_PERCENT_RES = 20.0
SNAPSHOT_PERCENT_HOM = 20.0
SEED = 42
...
def _load_ann_for_hprom(ann_dir, device):
    meta = np.load(os.path.join(ann_dir, "manifold_ann_metadata.npz"), allow_pickle=True)
    d_meta = np.load(os.path.join(ann_dir, "ann_dataset_metadata.npz"), allow_pickle=True)

    in_dim = int(np.ravel(meta["input_dim"])[0]) if "input_dim" in meta else int(np.ravel(d_meta["input_dim"])[0])
    out_dim = int(np.ravel(meta["output_dim"])[0]) if "output_dim" in meta else int(np.ravel(d_meta["n_secondary"])[0])
    if "include_macro_strain_input" in meta:
        include_macro = bool(int(np.ravel(meta["include_macro_strain_input"])[0]))
    else:
        include_macro = bool(int(np.ravel(d_meta["include_macro_strain_input"])[0]))
    if include_macro:
        raise RuntimeError(
    ...
    hidden_layers = tuple(int(v) for v in np.asarray(meta["hidden_layers"], dtype=np.int64).reshape(-1)) if "hidden_layers" in meta else (128, 256, 256, 256, 128)
    ...
    model = ManifoldANN(
        np.asarray(meta["x_mean"], dtype=np.float32),
        np.asarray(meta["x_std"], dtype=np.float32),
        np.asarray(meta["y_mean"], dtype=np.float32),
        np.asarray(meta["y_std"], dtype=np.float32),
        in_dim=in_dim,
        out_dim=out_dim,
    ...
    model.eval()
    return model, include_macro


def _evaluate_ann_qs_and_jac(q_p_val, ann_model, device, n_secondary, n_primary):
    q_p_t = torch.from_numpy(np.asarray(q_p_val, dtype=np.float32)).unsqueeze(0).to(device)
    with torch.enable_grad():
        q_in = q_p_t.clone().detach().requires_grad_(True)

        def _ann_wrap(q_loc):
            return ann_model.output_unscaler(ann_model.mlp(ann_model.input_scaler(q_loc)))

        q_s_t = _ann_wrap(q_in)
        jac = torch.autograd.functional.jacobian(_ann_wrap, q_in).reshape(n_secondary, n_primary)

    return (
        q_s_t.detach().cpu().numpy().reshape(-1).astype(float),
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="Stage 9a-ANN: build ECM dataset with separate residual/hom sampling."
    )
    p.add_argument("--snapshots-dir", type=str, default=SNAPSHOTS_DIR)
    p.add_argument("--basis-dir", type=str, default=BASIS_DIR)
    p.add_argument("--ann-dir", type=str, default=ANN_DIR)
...
def main():
    args = _parse_args()
    print("--- Stage 9a-ANN: Building ECM Dataset ---")
