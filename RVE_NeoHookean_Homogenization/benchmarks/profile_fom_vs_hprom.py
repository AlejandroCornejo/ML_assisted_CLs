...
def _load_hprom_model_and_data(
    model_dir=str(_HERE / "pod" / "stage_2_pod_rve"),
    # NOTE: "stage_5_hprom_data" predates the per-family ECM directory naming
    # used elsewhere in this repo and no longer exists; pass an existing ECM
    # directory explicitly (e.g. hprom/ann/ecm_fixed) to use this profiler.
...
def main():
    ...
    phi_f, free_dofs, dir_dofs, eq_map, Xc, Yc, ecm_data = _load_hprom_model_and_data(
        model_dir=str(_HERE / "pod" / "stage_2_pod_rve"),
        hprom_dir=str(_HERE / "hprom" / "ann" / "ecm_fixed"),
    )
