...
"""Stage 1 (FOM): run the stage-0 trajectory bundle with fom_solver_rve.py."""
...
def parse_args():
    p = argparse.ArgumentParser(description="Stage 1 FOM training launcher")
    p.add_argument(
        "--stage0-file",
        type=str,
    ...
    p.add_argument("--out-dir", type=str, default="stage_1_training_set_fom", help="Output root directory.")
    ...
    p.add_argument("--young-mpa", type=float, default=None, help="Override Young modulus.")
...
def run_single(args, idx, workdir):
    ...
    fom_solver_path = os.path.join(os.path.dirname(workdir), "core", "fom_solver_rve.py")
    cmd = [
        "python3",
        fom_solver_path,
    ...
    print(f"[STAGE1-FOM] Running Trajectory {idx}...")
    print(" ".join(cmd))
    result = subprocess.run(cmd, cwd=workdir)
    if result.returncode != 0:
        raise RuntimeError(
            f"Trajectory {idx} FAILED to converge (exit code {result.returncode}). "
...
def main():
    ...
    print(f"[STAGE1-FOM] Starting batch of {len(indices)} trajectories...")
    for i, idx in enumerate(indices, 1):
        print(f"\n{'='*60}")
        print(f"[STAGE1-FOM] Trajectory {idx}  ({i}/{len(indices)})")
        print(f"{'='*60}")
        try:
    ...
    print(f"\n[STAGE1-FOM] All {len(indices)} trajectories completed successfully.")
