"""Train one constrained multicavity feature-count-sweep model.

The runner is isolated from the historical m=32 campaigns.  It reads frozen
...
RULE = BASE / "protocol/feature_count_sweep_training_rule_v1.json"
...
def load_rule(features: Path, rule_path: Path = RULE):
    ...
    if (rule.get("id") != "material_B_feature_count_sweep_training_v1"
        ...
        raise ValueError("Invalid feature-count-sweep rule or preparation")
...
def _finish(output, state, model, adam, scheduler, lbfgs, scales, recipe, rule, rule_path):
    ...
    report = dict(status="complete", model=checkpoint["name"], seed=checkpoint["seed"],
        ...
        sweep_rule_sha256=digest(rule_path),
        best_validation_score=state["best_score"], best_origin=state["best_origin"],
...
def run(name, seed, output: Path, *, features: Path, rule_path: Path = RULE, resume=False,
    ...
    if name not in NAMES or seed not in rule["seeds"]:
        raise ValueError("Undeclared sweep model or seed")
    ...
    if resume:
        if not (output / base.CHECKPOINT_NAME).is_file():
            raise FileNotFoundError("No resumable sweep state")
        if (output / "failure.json").exists():
            raise RuntimeError("Failed sweep run requires inspection before resume")
    else:
