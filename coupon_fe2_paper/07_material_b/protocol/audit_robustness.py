...
def plot(rows):
    ...
    for color, name in zip(colors, names):
        ...
        for i, r in enumerate(group):
            c = r['collapse']
            # Omit only the reference W(1)=0 from this logarithmic display.
            axs[0].plot(c['J'][1:], np.array(c['energy_Pa'][1:])/1e6, color=color,
    ...
    axs[0].set(xscale='log', yscale='log', xlabel='Area ratio J (collapse to the right)',
    ...
    axs[1].legend(loc='lower left', framealpha=.8, fontsize=8)
    for ax in axs:
        ax.grid(alpha=.2)
