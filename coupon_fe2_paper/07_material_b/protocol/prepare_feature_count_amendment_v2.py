"""Freeze the low-count feature tables for the explicit multicavity amendment.
...
The existing m=8,16,24,32 tables remain untouched; this amendment creates
only m=2,4,6 from the same frozen fit/reference information.
"""
...
def _recipe(feature_count: int) -> dict:
    ...
    recipe["id"] = f"material_B_feature_count_amendment_v2_preparation_m{feature_count:02d}"
    recipe["feature_selection"]["count"] = feature_count
    recipe["models"]["names"] = ["ICNN-fixed", "ICNN-learned",
                                  "ICKAN-fixed", "ICKAN-learned"]
    recipe["timing"] = ("Explicit low-count multicavity sensitivity amendment. "
    ...
    recipe["scope"] = ("Preparation only for the low-count feature-count amendment: no Adam/LBFGS "
...
def _obsolete_m32_check(payload: dict) -> dict:
...
def prepare_one(feature_count: int, destination: Path, arrays: dict[str, np.ndarray]) -> dict:
    ...
    if feature_count not in COUNTS:
        raise ValueError("Undeclared amendment feature count")
    ...
    manifest = dict(
        ...
        protocol_id="material_B_feature_count_amendment_v2",
        ...
        feature_set_rule=("Each low count is selected independently by the inherited NNLS-support and "
...
def main() -> int:
    ...
    _atomic_json(args.out / "sweep_preparation_manifest.json", dict(
        status="complete", protocol_id="material_B_feature_count_amendment_v2",
        ...
        interpretation=("Frozen count-specific feature tables for a explicit low-count "
