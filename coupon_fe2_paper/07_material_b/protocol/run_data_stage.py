...
from run_preflight import boundary_screen, rim_indices  # noqa: E402
...
def enforce_physical_screen(fields: dict, boundary: dict, spec: dict) -> None:
    """Reject a converged algebraic solution that fails a frozen physical screen."""
    screen = spec["physical_screen"]
    if fields["min_micro_J"] <= screen["minimum_micro_J_exclusive"]:
        raise RuntimeError("Final state has a nonpositive microscopic determinant")
    if not all(np.isfinite(boundary[key])
               for key in ("min_polygon_gap", "max_periodic_jump_error")):
        raise RuntimeError("Final state has a nonfinite deformed-boundary diagnostic")
    if boundary["min_polygon_gap"] <= screen["minimum_deformed_polygon_gap_exclusive"]:
        raise RuntimeError("Final state violates the frozen deformed polygon-gap screen")
    if boundary["max_periodic_jump_error"] >= screen["maximum_periodic_jump_error_exclusive"]:
        raise RuntimeError("Final state violates the frozen periodic-jump screen")
    if (boundary["self_intersection"]
            and not screen["allow_deformed_polygon_self_intersection"]):
        raise RuntimeError("Final state has a deformed cavity-polygon self-intersection")
...
def consolidate(report: dict, output: Path) -> Path:
    ...
    polygon_gap = np.full(count, np.nan)
    self_intersection = np.zeros(count, dtype=bool)
    periodic_jump = np.full(count, np.nan)
    ...
    for position in range(count):
        ...
        with np.load(state_path(output, position), allow_pickle=False) as saved:
            ...
            polygon_gap[position] = saved["minimum_deformed_polygon_gap"]
            self_intersection[position] = saved["deformed_polygon_self_intersection"]
            periodic_jump[position] = saved["max_periodic_jump_error"]
    ...
    deterministic_npz(destination, dict(strain=strain, stress=stress, tangent=tangent,
        ...
        minimum_deformed_polygon_gap=polygon_gap,
        deformed_polygon_self_intersection=self_intersection,
        max_periodic_jump_error=periodic_jump,
...
def main() -> int:
    ...
    coordinate_path = BASE / spec["mesh_policy"][args.mesh + "_mesh_coordinates"]
    if digest(coordinate_path) != spec["mesh_policy"][args.mesh + "_mesh_coordinates_sha256"]:
        raise ValueError("Mesh-coordinate artifact differs from the frozen protocol")
    ...
    if digest(geometry_path) != spec["domain_basis"]["geometry_specification_sha256"]:
        raise ValueError("Geometry specification differs from the frozen protocol")
    geometry = json.loads(geometry_path.read_text())
    with np.load(coordinate_path, allow_pickle=False) as mesh_coordinates:
        xy = np.asarray(mesh_coordinates["xy"])
    ...
    if args.resume:
        ...
        sources = [args.spec, args.design, args.design_report, Path(__file__),
                   ...
                   BASE / "run_preflight.py", BASE / "audit_pilot.py", BASE / "geometry.py",
                   geometry_path, mesh_path, coordinate_path]
    ...
    with true_neo_hookean_active():
        rve = pf.PeriodicRVE(mesh_path.with_suffix(""),
                             cell_area=float(geometry["cell_side"])**2)
        for planned in pending[:limit]:
            ...
            try:
                ...
                fields, u, _pk1 = field_stats(rve, target, q)
                ...
                boundary = boundary_screen(xy, u[rve._eq_map], rims, target,
                                           float(geometry["cell_side"]))
                enforce_physical_screen(fields, boundary, spec)
                energy = float(rve.homogenized_energy())
                if (not np.isfinite(stress).all() or not np.isfinite(energy)
                        or (need_tangent and not np.isfinite(tangent).all())):
                    raise RuntimeError("Final constitutive output is nonfinite")
                record.update(ok=True, stress=np.asarray(stress).tolist(),
                              tangent=np.asarray(tangent).tolist() if need_tangent else None,
                              energy=energy, fields=fields, boundary=boundary, attempts=attempts,
                ...
                atomic_state(state_path(output, planned["plan_position"]), strain=target, q=q,
                    ...
                    minimum_deformed_polygon_gap=boundary["min_polygon_gap"],
                    deformed_polygon_self_intersection=boundary["self_intersection"],
                    max_periodic_jump_error=boundary["max_periodic_jump_error"],
