...
\begin{figure}[tbp]
 \centering\input{figures/integrated_hat_construction}
...
Let the features satisfy Eq.~\eqref{eq:featureconstraints}, let the core be either of the $C^2$ convex, componentwise nondecreasing constructions in Section~\ref{sec:convex_cores}, and let $\alpha,\beta,\varepsilon>0$. Then Eq.~\eqref{eq:energy} defines a $C^2$, objective, two-dimensional polyconvex energy on $\GL$. Its associated second Piola stress $\Sg$ is symmetric, its work-conjugate representation is $\ssv=\nabla_{\ee}W$, $W(\I)=0$, $\ssv(\bm0)=\bm0$, and $W$ diverges under collapse, unbounded expansion, and unbounded distortion at bounded positive area. Thus the construction satisfies (C1)--(C6).
\end{proposition}
...
\end{table}
\FloatBarrier
Section~\ref{sec:reduced} next defines the intrusive microscopic references used in the structural accuracy--cost comparison. Section~\ref{sec:cell} then evaluates whether the constitutive implementations reproduce the constructions established above and how accurately the resulting energies approximate the RVE response.
