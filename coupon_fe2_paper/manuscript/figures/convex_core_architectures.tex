...
\begin{tikzpicture}[x=1cm,y=1cm,font=\small,
 ...
 \node[align=center,text width=6.6cm] at (12.55,7.98)
   {\textcolor{teal!75!black}{Learned scalar connection functions}\\[2pt]
    \textcolor{teal!75!black}{(convex and nondecreasing)}\\[2pt]
    $\longrightarrow$ sum at nodes + biases};

 % ICNN: first and final hidden layers, repeated layers indicated by ellipses.
 \begin{scope}[xshift=1.6cm,yshift=1.35cm]
  \node[anchor=west,font=\small\bfseries] at (.12,5.55) {(a)};
  \node[font=\footnotesize] at (1.6,5.57) {Scalar core};
  \foreach \level in {1.7,4.05}
    \filldraw[red!65!black,fill=red!3,line width=.75pt]
      (.02,\level-.25) rectangle (3.18,\level+.25);
  \node[dot,label=below:{$y_1$}] (icinput1) at (.9,.55) {};
  \node[dot,label=below:{$y_m$}] (icinputm) at (2.3,.55) {};
  \node[font=\footnotesize,text=violet!65!black] (icinputvector) at (1.6,-.1)
    {$\bm y$};
  \node[dot] (icoutput) at (1.6,5.25) {};
  \foreach \j in {0,...,2} {
    \pgfmathsetmacro{\hiddenx}{.35+1.25*\j}
    \draw[weight] (icinput1) -- (\hiddenx,1.7);
    \draw[weight] (icinputm) -- (\hiddenx,1.7);
    \draw[weight] (\hiddenx,4.05) -- (icoutput);
    \foreach \k in {0,...,2} {
      \pgfmathsetmacro{\nextx}{.35+1.25*\k}
      \draw[weight] (\hiddenx,1.7) -- (\nextx,4.05);
    }
  }
  \fill[white] (.02,2.37) rectangle (3.23,2.92);
  \foreach \hiddenx in {.35,1.6,2.85}
    \node at (\hiddenx,2.65) {$\vdots$};
  ...
  \draw[inputmap,rounded corners=2pt] (icinputvector.east)
    -- (3.7,-.1) -- (3.7,5.25);
  \draw[maparrow] (3.7,5.25) -- (icoutput.east);
  \draw[inputmap] (3.7,3.53) -- (.35,3.53);
  \foreach \hiddenx in {.35,1.6,2.85}
    \draw[maparrow] (\hiddenx,3.53) -- (\hiddenx,3.85);
  \node[fill=white,inner sep=1pt,text=violet!65!black] at (3.7,2.65) {$\vdots$};
  \foreach \j in {0,...,2} {
    \pgfmathsetmacro{\hiddenx}{.35+1.25*\j}
    \pic[scale=.58] at (\hiddenx,1.7) {softplus};
 ...
 % ICKAN: learned function layers and sums, with only an output input skip.
 \begin{scope}[xshift=8.9cm,yshift=1.35cm]
  \node[anchor=west,font=\small\bfseries] at (.12,5.55) {(b)};
  \node[font=\footnotesize] at (1.6,5.57) {Scalar core};
  \foreach \level in {1.1,3.35,4.7}
    \filldraw[teal!70!black,fill=teal!3,line width=.75pt]
      (0,\level-.22) rectangle (3.26,\level+.22);
  \node[dot,label=below:{$y_1$}] (kaninput1) at (.9,.55) {};
  \node[dot,label=below:{$y_m$}] (kaninputm) at (2.3,.55) {};
  \node[font=\footnotesize,text=violet!65!black] (kaninputvector) at (1.6,-.1)
    {$\bm y$};
  \node[dot] (kanoutput) at (1.6,5.25) {};
  \foreach \j in {0,...,2} {
    \pgfmathsetmacro{\hiddenx}{.35+1.25*\j}
    \node[dot] (kanfirst\j) at (\hiddenx,1.7) {};
    \node[dot] (kanlast\j) at (\hiddenx,4.05) {};
  }
  \foreach \j in {0,...,2} {
    \pgfmathsetmacro{\hiddenx}{.35+1.25*\j}
    \draw[connection] (kanlast\j) -- (\hiddenx,4.7) -- (kanoutput);
    \foreach \r in {0,1} {
      \pgfmathsetmacro{\edgex}{.27+.53*(2*\j+\r)}
      \ifnum\r=0
        \draw[connection] (kaninput1) -- (\edgex,1.1) -- (kanfirst\j);
      \else
        \draw[connection] (kaninputm) -- (\edgex,1.1) -- (kanfirst\j);
      \fi
    }
    \foreach \k in {0,...,2} {
      \pgfmathsetmacro{\edgex}{.16+.36*(3*\j+\k)}
      \draw[connection] (kanfirst\k) -- (\edgex,3.35) -- (kanlast\j);
    }
  }
  \fill[white] (.02,2.37) rectangle (3.27,2.92);
  \foreach \hiddenx in {.35,1.6,2.85}
    \node at (\hiddenx,2.65) {$\vdots$};
  \foreach \j in {0,...,2} {
    \pgfmathsetmacro{\hiddenx}{.35+1.25*\j}
  ...
  \draw[callout] (3.29,4.7) to[out=0,in=170] (kanfunctions.west);
  \draw[callout] (3.0,4.05) to[out=0,in=130] (kansums.west);
 ...
 \node[font=\footnotesize] at (8.1,.29)
   {All linear weights are nonnegative; biases are unrestricted.};
\end{tikzpicture}
