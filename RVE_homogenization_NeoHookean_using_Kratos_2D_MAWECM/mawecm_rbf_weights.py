...
def eval_mawecm_rbf_with_jacobian(q_query, model, clip_nonnegative=True, renorm_target=None):
    ...
    if Pq.shape[1] > 0:
        Yh = Yh + Pq @ Beta
        if poly_mode == 2:
            # P = [1, q1, q2, ...] -> dP/dq is zero except linear terms.
            dP_dq = np.zeros((m_query, Pq.shape[1], q_dim), dtype=float)
            for a in range(q_dim):
                # linear term index 1+a corresponds to q_a
                if 1 + a < Pq.shape[1]:
                    dP_dq[:, 1 + a, a] = 1.0
            dYh_dq += np.einsum("mpa,pn->mna", dP_dq, Beta)

    # Unconstrained physical weights and gradients.
    W_tilde = (Yh.T * s[:, None])  # (n_weights, M)
    dW_tilde = np.transpose(dYh_dq, (1, 0, 2)) * s[:, None, None]  # (n_weights, M, q_dim)
    ...
    if bool(clip_nonnegative):
        mask_pos = W > 0.0
        W = np.maximum(W, 0.0)
        # Piecewise derivative: zero where clipped.
        dW = dW * mask_pos[:, :, None]
    ...
    if renorm_target is not None:
        ...
        for m in range(m_query):
            ...
            if sw <= 1.0e-30:
                ...
                dW[:, m, :] = 0.0
                continue
            dsw = np.sum(dW[:, m, :], axis=0)  # (q_dim,)
            fac = target / sw
            # Quotient rule:
            # w_hat = target * w / sum(w)
            # dw_hat = target/sum * dw - target/sum^2 * w * dsum
