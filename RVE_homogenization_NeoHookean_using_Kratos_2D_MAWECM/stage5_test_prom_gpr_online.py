...
"""Stage 5 (online): run FOM and PROM-GPR on a test trajectory unseen by POD training.

Default behavior builds a new "unseen" trajectory in parameter space (mu), maps it
into strain waypoints, runs FOM (iterative) and PROM-GPR (sparse-GP decoder), and
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Stage5 online: FOM vs PROM-GPR on unseen test trajectory")
    ...
    p.add_argument(
        "--prom-fail-on-nonconvergence",
        type=int,
        default=1,
        choices=[0, 1],
        help="If 1, any non-converged Newton step aborts the run.",
    )

    p.add_argument("--out-dir", type=str, default="stage_5_online_prom_gpr")
...
def main():
    ...
    print("=" * 78)
    print("Stage 5 online: FOM vs PROM-GPR")
    print("=" * 78)
    ...
    if args.run_prom_gpr or not (os.path.exists(prom_eps_file) and os.path.exists(prom_sig_file)):
        print("\n[RUN] PROM-GPR (online surrogate + reduced corrector)...")
    ...
    summary = {
        ...
        "prom_corrector_damping_after_iter": int(args.prom_corrector_damping_after_iter),
        "prom_corrector_damping_factor": float(args.prom_corrector_damping_factor),
        "prom_corrector_l2_reg": float(args.prom_corrector_l2_reg),
        "stage2a_dir": args.stage2a_dir,
        "stage2b_dir": args.stage2b_dir,
        "stage3_dataset_file": args.stage3_dataset_file,
        "prom_fail_on_nonconvergence": int(args.prom_fail_on_nonconvergence),
        "stage4_gpr_dir": args.stage4_gpr_dir,
    }
    ...
    print(f"  PROM-GPR source     : {prom_source}")
