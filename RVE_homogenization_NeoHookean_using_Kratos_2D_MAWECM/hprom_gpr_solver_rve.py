...
"""HPROM-GPR solver (2D-MAWECM, classical ECM hyper-reduction).
...
def RunHpromGprBatchSimulation(
    ...
    assembler_hr = VectorizedAssembler(
        ...
        log_label="HPROM-GPR-ResidualAssembler",
    )
    assembler_hom = VectorizedAssembler(
        ...
        elements=elem_hom,
        element_scales=None,
        log_label="HPROM-GPR-HomAssembler",
    )
    ...
    print(
        f"  [HPROM-GPR] Solving trajectory with {n_steps_total} increments "
        f"(reduced corrector iters={n_corr})"
    )
    print(f"  [HPROM-GPR] Residual support: |Z_res|={z_res.size} ({100.0*z_res.size/max(n_elem,1):.1f}% of {n_elem})")
    print(
        f"  [HPROM-GPR] Hom support: |Z_hom|={z_hom.size} ({100.0*z_hom.size/max(n_elem,1):.1f}% of {n_elem}), "
        f"mesh_mode={mesh_mode}"
    )
    print(
        f"  [HPROM-GPR] DOF overlap full->mesh: free_used={n_overlap}/{free_dofs_full.size}"
    )
    print("  [HPROM-GPR] Reduced Newton uses same knobs as PROM-GPR.")
    ...
    for step in range(1, n_steps_total + 1):
        ...
        for it in range(n_corr + 1):
            ...
            q_s = q_s_raw - q_s0_raw - (j0_raw @ q_m)
            q_master = a_m @ q_m
        ...
        if not converged:
            ...
            if quasi_converged:
                ...
                msg = (
                    f"[HPROM-GPR] Step {step:03d}/{n_steps_total} not converged in {n_corr} iters. "
                ...
                print(f"  [HPROM-GPR][WARN] {msg}")
                if nonfinite_detected:
                    print("  [HPROM-GPR][WARN] non-finite state detected; rolled back to best finite iterate.")
            k_red_old = None
        ...
        print(
            f"  [HPROM-GPR] Step {step:03d}/{n_steps_total}: "
