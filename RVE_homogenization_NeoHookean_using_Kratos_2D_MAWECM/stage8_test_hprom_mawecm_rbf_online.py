...
"""Stage 8 (2D-MAWECM): online comparison FOM vs PROM-RBF vs HPROM-MAWECM-RBF."""
...
from hprom_mawecm_rbf_solver_rve import LoadHpromMawEcmRbfModel, RunHpromMawEcmRbfBatchSimulation


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Stage8 online: FOM vs PROM-RBF vs HPROM-MAWECM-RBF")
    ...
    p.add_argument("--run-prom-rbf", action="store_true")
    p.add_argument("--run-hprom-mawecm-rbf", action="store_true")
    ...
    p.add_argument(
        ...
        type=str,
        default="ecm_fixed",
        choices=["ecm_fixed", "full_fom", "maw_single"],
    )
    ...
    p.add_argument(
        ...
        default=1,
        choices=[0, 1],
        help="If enabled and MAW file contains hrom_mesh_base, HPROM-MAW runs on that reduced mdpa.",
    )

    p.add_argument("--out-dir", type=str, default="stage_8_online_hprom_mawecm_rbf")
...
def _maybe_plot(out_dir: str, eps_f, sig_f, eps_p, sig_p, eps_h, sig_h):
    ...
    for i, ylab, xlab, suf in labels:
        ...
        ax.plot(eps_h[:, i], sig_h[:, i], "b:", lw=1.8, label="HPROM-MAWECM-RBF")
        ax.set_xlabel(xlab)
        ax.set_ylabel(ylab)
    ...
    ax.plot(err_h, "b-", lw=1.3, label="HPROM-MAWECM-RBF rel. stress error")
    ax.set_yscale("log")
...
def main():
    ...
    print("=" * 80)
    print("Stage 8 online: FOM vs PROM-RBF vs HPROM-MAWECM-RBF")
    print("=" * 80)
    ...
    if args.run_prom_rbf or not (os.path.exists(p_eps_file) and os.path.exists(p_sig_file)):
        print("\n[RUN] PROM-RBF")
        prom_pack = LoadPromRbfModel(
        ...
        t0 = time.perf_counter()
        eps_p, sig_p = RunPromRbfBatchSimulation(
            params_prom,
    ...
    if args.run_hprom_mawecm_rbf or not (os.path.exists(h_eps_file) and os.path.exists(h_sig_file)):
        print("\n[RUN] HPROM-MAWECM-RBF")
        prom_pack_h, maw_pack = LoadHpromMawEcmRbfModel(
            stage2a_dir=args.stage2a_dir,
            stage2b_dir=args.stage2b_dir,
            stage3_dataset_file=args.stage3_dataset_file,
            stage4_rbf_dir=args.stage4_rbf_dir,
        ...
        t0 = time.perf_counter()
        eps_h, sig_h = RunHpromMawEcmRbfBatchSimulation(
            params_hprom,
    ...
    summary = {
        ...
        "source_fom": src_f,
        "source_prom_rbf": src_p,
        "source_hprom_mawecm_rbf": src_h,
        "mesh_fom": mesh_fom,
        "mesh_prom": mesh_prom,
        "mesh_hprom": mesh_hprom,
        "mesh_hprom_mode": mesh_hprom_mode,
        "runtime_sec_fom": t_f,
        "runtime_sec_prom_rbf": t_p,
        "runtime_sec_hprom_mawecm_rbf": t_h,
        "speedup_fom_over_prom_rbf": sp_prom,
        "speedup_fom_over_hprom_mawecm_rbf": sp_hprom,
        "stage2a_dir": args.stage2a_dir,
        "stage2b_dir": args.stage2b_dir,
        "stage3_dataset_file": args.stage3_dataset_file,
        "stage4_rbf_dir": args.stage4_rbf_dir,
    ...
    if np.isfinite(err_qm_ph):
        print(f"  rel q_m error HPROM-MAW vs PROM-RBF   : {err_qm_ph:.3e}")
    if np.isfinite(err_qs_ph):
        print(f"  rel q_s error HPROM-MAW vs PROM-RBF   : {err_qs_ph:.3e}")
    if t_f is not None:
        print(f"  runtime FOM [s]                       : {t_f:.3f}")
    if t_p is not None:
        print(f"  runtime PROM-RBF [s]                  : {t_p:.3f}")
    if t_h is not None:
