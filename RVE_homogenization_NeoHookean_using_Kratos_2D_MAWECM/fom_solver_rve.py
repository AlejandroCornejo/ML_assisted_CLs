...
class VectorizedAssembler:
    ...
    def __init__(self, mp, n_dof, eq_map, elements=None, element_scales=None, log_label="VectorizedAssembler"):
        ...
        if self.n_elems == 0:
            self.n_gauss = 0
            self.element_scales = np.zeros(0, dtype=float)
            self.w_detJ = np.zeros((0, 0), dtype=float)
            self._w_detJ_base = np.zeros((0, 0), dtype=float)
            self.area_e = np.zeros(0, dtype=float)
            return
        ...
        self._w_detJ_base = self.w_detJ.copy()
    ...
    def SetElementScales(self, element_scales):
        ...
        if self.n_elems == 0:
            return
        scales = np.asarray(element_scales, dtype=float).reshape(-1)
        if scales.size != self.n_elems:
            raise ValueError(
                f"element_scales size {scales.size} does not match "
                f"number of elements {self.n_elems}."
            )
