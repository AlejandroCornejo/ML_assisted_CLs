...
"""HPROM-MAWECM-RBF solver (2D-MAWECM).
...
def RunHpromMawEcmRbfBatchSimulation(
    parameters,
    ...
    if rbf_input_space != "q_m":
        raise RuntimeError(
            "HPROM-MAWECM q_m-Newton requires Stage4 RBF trained with '--rbf-input-space q_m'. "
    ...
    maw_apply_to_hom = bool(int(np.ravel(mawecm_data["maw_apply_to_hom"])[0])) if "maw_apply_to_hom" in mawecm_data else False
    ...
    if hom_mode not in ("ecm_fixed", "full_fom", "maw_single"):
        raise RuntimeError(
            f"Unsupported homogenization_mode='{homogenization_mode}'. "
            "Use 'ecm_fixed', 'full_fom', or 'maw_single'."
        )
    if hom_mode == "maw_single" and (not maw_apply_to_hom):
        raise RuntimeError(
            "Requested homogenization_mode='maw_single' but MAW file does not enable hom adaptation. "
            "Rebuild Stage8b with '--maw-mode single_res_hom'."
        )
    ...
    if hom_mode == "full_fom":
    ...
    elif hom_mode == "maw_single":
        z_hom_eps = np.asarray(z_res, dtype=np.int64).reshape(-1)
        if z_hom_eps.size == 0:
            raise RuntimeError("maw_single requires non-empty residual support.")
        z_hom_sig = z_hom_eps.copy()
        elem_hom_eps = [elements_all[int(i)] for i in z_hom_eps.tolist()]
        elem_hom_sig = [elements_all[int(i)] for i in z_hom_sig.tolist()]
        ...
        z_hom_eps = np.asarray(z_eps, dtype=np.int64).reshape(-1)
        z_hom_sig = np.asarray(z_sig, dtype=np.int64).reshape(-1)
        if z_hom_eps.size == 0:
            z_hom_eps = np.flatnonzero(np.abs(w_eps_full) > _W_TOL).astype(np.int64)
        if z_hom_sig.size == 0:
            z_hom_sig = np.flatnonzero(np.abs(w_sig_full) > _W_TOL).astype(np.int64)
        if z_hom_eps.size == 0:
            z_hom_eps = np.arange(n_elem, dtype=np.int64)
        if z_hom_sig.size == 0:
            z_hom_sig = np.arange(n_elem, dtype=np.int64)

        elem_hom_eps = [elements_all[int(i)] for i in z_hom_eps.tolist()]
        elem_hom_sig = [elements_all[int(i)] for i in z_hom_sig.tolist()]
        w_eps_sel = w_eps_full[z_hom_eps]
        w_sig_sel = w_sig_full[z_hom_sig]
    ...
    print(
        f"  [HPROM-MAWECM-RBF] Solving trajectory with {n_steps_total} increments "
        f"(corrector iters={n_corr}, update_weights_each_iter={int(update_each_iter)})"
    )
    print(
        f"  [HPROM-MAWECM-RBF] Residual support: |Z_res|={z_res.size} "
        f"({100.0*z_res.size/max(n_elem,1):.1f}% of {n_elem})"
    )
    if hom_mode == "full_fom":
        print(
            f"  [HPROM-MAWECM-RBF] Hom support: full mesh "
            f"({n_elem}/{n_elem}, 100.0%), mesh_mode={mesh_mode}"
        )
    else:
        z_hom_union = np.union1d(z_hom_eps, z_hom_sig).astype(np.int64)
        print(
            f"  [HPROM-MAWECM-RBF] Hom supports: |Z_eps|={z_hom_eps.size}, |Z_sig|={z_hom_sig.size}, "
            f"|Z_union|={z_hom_union.size} ({100.0*z_hom_union.size/max(n_elem,1):.1f}% of {n_elem}), mesh_mode={mesh_mode}"
        )
    print(
        f"  [HPROM-MAWECM-RBF] DOF overlap full->mesh: free_used={n_overlap}/{free_dofs_full.size}"
    )
    print(f"  [HPROM-MAWECM-RBF] MAW state space: {maw_state_space}")
    print(f"  [HPROM-MAWECM-RBF] Homogenization mode: {hom_mode}")
    print(f"  [HPROM-MAWECM-RBF] MAW applies to hom: {int(maw_apply_to_hom)}")
    print(f"  [HPROM-MAWECM-RBF] Include d(w)/d(q_m) tangent term: {int(use_weight_tangent)}")
    ...
    for step in range(1, n_steps_total + 1):
        ...
        if not converged:
            ...
            dq_quasi = (
                ...
                and (
                    (ndq_last <= float(prom_corrector_dq_abs_tol))
                    or (np.isfinite(dq_rel_last) and (dq_rel_last <= float(prom_corrector_dq_rel_tol)))
                )
            )
            if quasi_converged:
                q_m = q_m_best.copy()
                converged = True
                print(
                    "  [HPROM-MAWECM-RBF] Step accepted as quasi-converged: "
                    f"best ||R_r||={nr_best:.3e}, rel={rrel_best:.3e}"
                )
            elif dq_quasi:
                converged = True
                print(
                    "  [HPROM-MAWECM-RBF] Step accepted as dq-quasi-converged: "
                    f"last ||R_r||={nr_last:.3e}, ||dq_m||={ndq_last:.3e}, dq_rel={dq_rel_last:.3e}"
                )
            else:
                ...
                print(
                    f"  [HPROM-MAWECM-RBF][WARN] Step {step:03d}/{n_steps_total} not converged in {n_corr} iters. "
                    f"last ||R_r||={nr_last:.3e}, ||dq_m||={ndq_last:.3e}"
                )
                if nonfinite_detected:
                    print("  [HPROM-MAWECM-RBF][WARN] non-finite state detected; rolled back to best finite iterate.")
            k_red_old = None
        ...
        if hom_mode == "full_fom":
            ...
            w_hom = np.asarray(w_last if w_last is not None else _eval_maw_weights(_state_for_maw(maw_state_space, q_m)), dtype=float)
            eps_h, _ = CalculateHomogenizedFromAssemblerWithElementWeights(
                assembler_hom_eps,
                w_eps=w_hom,
                w_sig=None,
                reference_measure=float(a0_ref),
            )
            _, sig_h = CalculateHomogenizedFromAssemblerWithElementWeights(
                assembler_hom_sig,
                w_eps=None,
                w_sig=w_hom,
                reference_measure=float(a0_ref),
        ...
        print(
            f"  [HPROM-MAWECM-RBF] Step {step:03d}/{n_steps_total}: "
            f"||q_m||={np.linalg.norm(q_m):.3e}, ||q_s||={np.linalg.norm(q_s):.3e}, "
    ...
    if out_dir is not None:
        tag = f"trajectory_{trajectory_index}" if trajectory_index is not None else "hprom_mawecm_rbf_run"
